...
def test_create_success_with_splash(requireMocking):
    jsonString = json.dumps({'method': 'test_lib', 'mz_rt': '12_1', 'sample': 'tgtTest',
                             'splash': 'splash10-a112-0123456789-a0s1d2f3g4h5j6k7l8q9'})
...
def test_create_invalid_splash(requireMocking):
    jsonString = json.dumps({'method': 'test_lib', 'mz_rt': '12_1', 'sample': 'tgtTest', 'splash': 'splash-blah-blah'})

    result = create.create({'body': jsonString}, {})
    assert 422 == result['statusCode']
    assert 'body' in result
    print(result['body'])
    assert 'Error: \'splash-blah-blah\' does not match \'^splash1\\\\d-[0-9a-z]{4}-[0-9]{10}-[0-9a-z]{20}$\'' \
           in result['body'].split('\n')
